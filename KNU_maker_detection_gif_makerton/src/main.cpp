/*
   assimp, glfw, gl3w 라이브러리를 프로젝트 경로 밖에서 추가해야 함.
   이 세가지 lib는 apt, brew 등으로 쉽게 설치할 수 있어어 소스를 추가하지 않음.
 */
...
int main(int, char**) {
	...
	GLFWwindow* window = glfwCreateWindow(1280, 720, "GIF ArucoMarker_Visualization", NULL, NULL);
	if (window == NULL)
		return 1;
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if(cvCam.getCamImage(imgShow)){
			...
			if(cvCam.isOpened() && cvCam.getMarkerPose(2, matView, imgShow)) {
		...
		// Opengl에는 Projection, View, Model matrix(행렬)을 기본으로 사용함.
		// FOV 는 45.0도...
